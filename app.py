...
def login():
    if request.method == 'POST':
        ...
        # Verifikasi apakah NIP dan nama lengkap sesuai dengan data dosen dalam database
        dosen = DataDosen.query.filter_by(nip=nip, nama_lengkap=nama_lengkap).first()
        
        if dosen:
            # Jika sesuai, buat token akses untuk dosen
            access_token = create_access_token(identity={'nip': nip, 'nama_lengkap': nama_lengkap})
            return jsonify({'access_token': access_token}), 200
        else:
            # Jika tidak sesuai, kembalikan pesan error
            return jsonify({"message": "Invalid NIP atau nama"}), 401
